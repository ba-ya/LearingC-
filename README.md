# LearingC++

平台：力扣

[时间复杂度](https://blog.csdn.net/qq_41523096/article/details/82142747)

# 2021

[a little](./2021/2021.md)

# 25.10-

## [基础算法](https://github.com/EndlessCheng/codeforces-go/tree/master/leetcode)

| 类型                       | 题目数目(169) | 通过次数 | 卡壳的题 |
| :------------------------: | :-----------: | :------: | :-------: |
| TwoPointersTowards1        | 5             | 3        |  |
| TwoPointersTowards2        | 4             | 3        |          |
| SlidingWindow              | 12            | 3        |          |
| BinarySearch1              | 11           | 3        | 275,2439,2517, |
| BinarySearch2              | 6             | 3        | 162,153,74,1901,154 |
| LinkedList_Reverse         | 6             | 3        | **check** |
| LinkedList_QuickNSlow      | 5             | 3        | 142,143 |
| LinkedList_Delete          | 7             | 3        | 82,2487 |
| BinaryTree_Understand      | 7             | 3        | 111,404,129,987 |
| BinaryTree_Apply           | 8             | 2        |  |
| BinaryTree_ThreeOrder      | 9             | 2        |          |
| BinaryTree_Ancestor        | 3             | 2        |          |
| BinaryTree_BFS             | 8             | 2        |          |
| BackTrack_Subset           | 4             | 2        |          |
| BackTrack_CombinationNPrun | 5             | 2        |          |
| BackTrack_Permutation      | 4             | 2        |          |
| DynamicProgramming         | 9             | 2        |          |
| ZeroOneBackbag             | 6             | 2        | 494,322 |
| LCS                        | 4             | 2        | 72,97,1092 |
| LIS                        | 6             | 2 | 300,1671,354,1187 |
| DP_BuyNSellStock           | 6             | 1        |          |
| DP_Interval                | 6             | 1 |          |
| DP_Tree                    | 7             | 1 |          |
| DP_MaxSet                  | 3             | 1 |          |
| DP_MinSet                  | 1             | 1 |          |
| Monotonic_Stack            | 10            | 2 | 901,1944,1793 |
| Monotonic_Queue            | 6             | 1 |          |

## [HOT100](https://leetcode.cn/studyplan/top-100-liked/)

大部分题目与上面的重复,

只新增2个文件补充**基础**里没写过的

|     类型     | 题目数目(100) | 通过次数 |            卡壳的题            |
| :----------: | :-----------: | :------: | :----------------------------: |
|     哈希     |       3       |    3     |                                |
|    双指针    |       4       |    3     |              42,               |
|   滑动窗口   |       2       |    3     |              438               |
|     子串     |       3       |    3     |             239,76             |
|   普通数组   |       5       |    3     |              53,               |
|     矩阵     |       4       |    3     |             73,54              |
|     链表     |      14       |    2     |     160,24,138,148,23,146      |
|    二叉树    |      15       |    2     | 111,226,108,98,230,199,114,437 |
|     图论     |       4       |    2     |          200,207,208           |
|     回溯     |       8       |    2     |           17,22,79,            |
|   二分查找   |       6       |    2     |            35,153,4            |
|      栈      |       5       |    2     |           20,155,394           |
|      堆      |       3       |    2     |          215,347,295           |
|   贪心算法   |       4       |    2     |           55,45,763            |
|   动态规划   |      10       |    2     |         279,139,152,32         |
| 多维动态规划 |       5       |    2     |           64,5,1143,           |
|     技巧     |       5       |    2     |       136,169,75,31,287        |

